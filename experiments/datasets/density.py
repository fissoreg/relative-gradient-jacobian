...
@torch.no_grad()
def plot(model, potential_or_sampling_fn, name, device, dist_name):
    n_pts = 1000
    if dist_name == 'smile':
        range_lim = [-6, 6]
    ...
    # save
    plt.savefig('vis_{}.png'.format(name))
    plt.close()
...
def plot_samples(samples_fn, ax, range_lim, n_pts):
    ...
    ax.hist2d(samples[:,0], samples[:,1], range=[range_lim, range_lim], bins=n_pts)  # , cmap=plt.cm.jet)
...
def plot_fwd_flow_density(model, ax, test_grid, n_pts, batch_size):
    ...
    data = [pt_to_jnp(batch) for batch in zz.split(batch_size, dim=0)]
    log_prob = model(data)
    prob = jnp.exp(log_prob)
    # plot
    ax.pcolormesh(xx, yy, prob.reshape((n_pts,n_pts)))  # , cmap=plt.cm.jet)
    ax.set_facecolor(plt.cm.jet(0.))
    ax.set_title('Predicted density')


class DENSITY(Dataset):
    def __init__(self, name, dataset_size=25000):
        self.label_size = 1
    ...
    def plot(self, model, name, device):
        plot(model, self.sample_function, name, device, dist_name=self.name)
